...
const ResumeForm = () => {
  ...
  // Fetching existing resume data if in edit mode.
  const getResume = useCallback(async () => {
    if (resumeId) {
      try {
        ...
        alert('Error fetching resume: ' + (error.response?.data?.detail || 'Unknown error'));
      }
    }
  }, [resumeId, reset]);

  // On mount, check for optimized resume data or fetch existing one
  useEffect(() => {
    if (location.state?.optimizedResume) {
      const transformedOptimized = {
        ...location.state.optimizedResume,
        projects: location.state.optimizedResume.projects.map((project) => ({
          ...project,
          used_skills: project.used_skills ? project.used_skills.join(', ') : '',
        })),
  ...
  }, [location.state, resumeId, reset, getResume]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      alert('Error saving resume: ' + (error.response?.data?.detail || 'Unknown error'));
    }
  };
  ...
  // Render personal information section using Material-UI TextFields
  ...
  // In edit mode, render the two-column layout
  return (
